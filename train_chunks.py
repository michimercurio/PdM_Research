...
def plot_loss(loss_over_time):
    ...
    plt.plot(loss_over_time['encoder/decoder'], label='Encoder/Decoder Loss')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
...
def train_model(epochs,
    ...
    if args.use_discriminator:
        ...
        # optimizer_autoencoder = optim.RMSprop(args.model.parameters(), lr=args.LR)
        # optimizer_autoencoder = optim.SGD(args.model.parameters(), lr=args.LR, momentum=0.9)
        # optimizer_autoencoder = optim.Adagrad(args.model.parameters(), lr=args.LR)
        # optimizer_autoencoder = optim.Adadelta(args.model.parameters(), lr=args.LR)
        # optimizer_autoencoder = optim.Nadam(args.model.parameters(), lr=args.LR)
    ...
    for epoch in range(epochs):
        ...
        if args.use_discriminator:
            ...
            #writer.add_scalar(loss_over_time, epoch)
...
def load_parameters(arguments):
    ...
    arguments.data_folder = "pompe_dati/"
    ...
    if arguments.FEATS == "noflow_feats":
        ...
        train_set = ChunkDataset("pompe_dati/training_chunks_mio.pkl")
        test_set = ChunkDataset("pompe_dati/test_completo/test_chunks_mio_completo.pkl")
    ...
    if arguments.use_discriminator:
        ...
        if "tcn" in arguments.MODEL_NAME:
            ...
            arguments.model = MODELS[arguments.MODEL_NAME](14,
                # arguments.NUMBER_FEATURES,
                                                           arguments.EMBEDDING,
                                                           arguments.DROPOUT,
                                                           arguments.LSTM_LAYERS,
...
def main(arguments):
    ...
    if models_exist and not arguments.force_training:
        if arguments.use_discriminator:
            ...
            arguments.model.load_state_dict(th.load(arguments.model_saving_string("AE"), map_location=th.device('cpu')))
    ...
    end_time = time.time()
    # print("Il tempo di training è stato di " + str(end_time_training - start_time_training) + " mentre il tempo complessivo è stato di " + str(end_time - start_time))
    print("Done")
